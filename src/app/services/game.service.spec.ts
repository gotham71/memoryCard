...
describe('GameService', () => {
  ...
  it('should set current preset based on user level', () => {
    const level = '5';
    spyOn(userService, 'getLevel').and.returnValue(level);
    gameService.getCurrentPreset();
    expect(gameService['level']).toBe(level);
    expect(gameService['currentPreset']).toBeDefined();
  });
  ...
  it('should check if try option is correct', () => {
    ...
    const result = gameService.checkTryIfCorrect(tryOption);
